...
fly -t ${FLY_TARGET} sp -p "${FLY_TARGET}-${PRODUCT_NAME}-patch" \
-c ./patch-product.yml \
...
## fly -t demo dp -p pcfdemo-cf-patch
## fly -t demo clear-task-cache -j pcfdemo-cf-patch/upload-and-stage-product -s download-product-from-s3
